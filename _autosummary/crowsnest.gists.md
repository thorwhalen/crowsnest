...
runner as [`crowsnest.actions`](crowsnest.actions.md#module-crowsnest.actions)) for every question of one message at a time, and keeps
...
`sessions` is what [`crowsnest.questions.scan()`](crowsnest.questions.md#crowsnest.questions.scan) returns. Returns counts:
