...
([`crowsnest.attention`](crowsnest.attention.md#module-crowsnest.attention)), and three surfaces compute that revision on their own: the
report renders it into the page ([`crowsnest.tools.report()`](crowsnest.tools.md#crowsnest.tools.report)), the attention verbs pin
it ([`crowsnest.tools.seen()`](crowsnest.tools.md#crowsnest.tools.seen) and the rest), and the watcher computes it again to say
that an item put off has woken ([`crowsnest.watch.attention_wakes()`](crowsnest.watch.md#crowsnest.watch.attention_wakes)). They agree only
...
([`crowsnest.config.report_settings()`](crowsnest.config.md#crowsnest.config.report_settings)) names the ledger directory, so
...
`resolvers` turn what a session wrote into links ([`crowsnest.links`](crowsnest.links.md#module-crowsnest.links));
`verdicts` and `owner` classify it ([`crowsnest.triage`](crowsnest.triage.md#module-crowsnest.triage)); `identity` and
`material` name the item and say what counts as a change to it
([`crowsnest.attention`](crowsnest.attention.md#module-crowsnest.attention)). `None` is each module’s own default, so
...
The row’s item id ([`crowsnest.attention.item_id()`](crowsnest.attention.md#crowsnest.attention.item_id)).
...
The row’s revision ([`crowsnest.attention.fingerprint()`](crowsnest.attention.md#crowsnest.attention.fingerprint)).
...
[`crowsnest.tools.resolve()`](crowsnest.tools.md#crowsnest.tools.resolve) takes one. Raises `KeyError` when none matches.
