...
it is reachable without editing anything: [`crowsnest.tools.lineage()`](crowsnest.tools.md#crowsnest.tools.lineage) carries
...
`events.jsonl`: that log rotates at 4 MiB ([`crowsnest.hook.MAX_EVENT_BYTES`](crowsnest.hook.md#crowsnest.hook.MAX_EVENT_BYTES))
...
[`crowsnest.watch.hook_event()`](crowsnest.watch.md#crowsnest.watch.hook_event) and every existing reader skip it unchanged: an
...
[`crowsnest.tools.sessions()`](crowsnest.tools.md#crowsnest.tools.sessions) marks the home it was asked for.
...
taken. [`crowsnest.tools.backfill_lineage()`](crowsnest.tools.md#crowsnest.tools.backfill_lineage) passes one.
...
[`crowsnest.tools.roster()`](crowsnest.tools.md#crowsnest.tools.roster) computes knowing which home it read; it is `''` for
...
but rotates ([`crowsnest.hook.MAX_EVENT_BYTES`](crowsnest.hook.md#crowsnest.hook.MAX_EVENT_BYTES)), so old names fall off the end;
...
- **The session, by its id.** A name does not pick out one session: two sessions may
  ...
  [`crowsnest.tools.resolve()`](crowsnest.tools.md#crowsnest.tools.resolve) matches it exactly, and it needs no quoting. Only a
