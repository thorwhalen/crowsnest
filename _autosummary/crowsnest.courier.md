...
A page published with a console store ([`crowsnest.report.ConsoleStore`](crowsnest.report.md#crowsnest.report.ConsoleStore)) writes what
...
2. Take the attention documents into this machine’s attention store, last write winning
   by `updated_at` ([`crowsnest.attention.import_docs()`](crowsnest.attention.md#crowsnest.attention.import_docs)), and write back what
...
[`crowsnest.publish`](crowsnest.publish.md#module-crowsnest.publish) sends the page), [`dir_copy()`](#crowsnest.courier.dir_copy) for a directory.
...
`None`). `recap` and `live` default to [`crowsnest.tools.recap()`](crowsnest.tools.md#crowsnest.tools.recap) and
[`crowsnest.tools.live()`](crowsnest.tools.md#crowsnest.tools.live), and exist so a test need not read real sessions.
