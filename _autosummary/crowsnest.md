...
1. **The roster** ([`crowsnest.tools.roster()`](crowsnest.tools.md#crowsnest.tools.roster)): who is alive, busy, idle or waiting,
   where, since when. Instant; no transcript is read.
2. **The activity** ([`crowsnest.tools.show()`](crowsnest.tools.md#crowsnest.tools.show), [`crowsnest.tools.turns()`](crowsnest.tools.md#crowsnest.tools.turns)): what a
   ...
   not enough, and [`crowsnest.tools.brief()`](crowsnest.tools.md#crowsnest.tools.brief) looks up openloops’ dated digest of a
...
3. **The ask**: a running session can be *messaged* and will answer from its own
   ...
   [`crowsnest.init`](crowsnest.init.md#module-crowsnest.init) is what sets a session up to live by them.

And one stream: [`crowsnest.watch.events()`](crowsnest.watch.md#crowsnest.watch.events) yields a line every time a session starts,
...
`Notification` hooks ([`crowsnest.hook`](crowsnest.hook.md#module-crowsnest.hook)), those two moments are pushed into the
...
whom\*\* ([`crowsnest.lineage`](crowsnest.lineage.md#module-crowsnest.lineage)). A fleet of forty reads as a list of forty until the
...
[`crowsnest.tools.lineage()`](crowsnest.tools.md#crowsnest.tools.lineage) reads the forest back. What older sessions left behind is
recovered once, and marked as the inference it is, by
[`crowsnest.tools.backfill_lineage()`](crowsnest.tools.md#crowsnest.tools.backfill_lineage).

Between the roster and the transcript there is a fourth thing, the only one crowsnest
authors: the **ledger** ([`crowsnest.ledger`](crowsnest.ledger.md#module-crowsnest.ledger)), one small markdown file per session,
...
([`crowsnest.attention`](crowsnest.attention.md#module-crowsnest.attention)) – which items they have seen, put off until later, marked
...
spawn records, the attention records (all four under [`crowsnest.paths.data_dir()`](crowsnest.paths.md#crowsnest.paths.data_dir)),
...
`row_context` reaches `attention_wakes()` ([`crowsnest.rows.RowContext`](crowsnest.rows.md#crowsnest.rows.RowContext)),
...
* **Return type:**
  [`Activity`](crowsnest.activity.md#crowsnest.activity.Activity)
...
* **Return type:**
  [`list`](https://docs.python.org/3/builtins/stdtypes.html#list)[[`Turn`](crowsnest.activity.md#crowsnest.activity.Turn)]
...
a home instead of spelling it ([`crowsnest.account.account_home()`](crowsnest.account.md#crowsnest.account.account_home), which also
...
`lineage.jsonl` ([`crowsnest.lineage.record_spawn()`](crowsnest.lineage.md#crowsnest.lineage.record_spawn); `lineage_path` is where,
...
| [`gists`](crowsnest.gists.md#module-crowsnest.gists)         | Gists for *Your questions* (#129): each question in a few words, and the answer it got.                                              |
