...
`~/.config/crowsnest/config.toml` first ([`crowsnest.config`](crowsnest.config.html.md#module-crowsnest.config), the naming scheme
...
`CLAUDE*` markers ([`crowsnest.spawn`](crowsnest.html.md#crowsnest.spawn) strips those); things that would override
...
([`crowsnest.config.claude_bin_setting()`](crowsnest.config.html.md#crowsnest.config.claude_bin_setting)) – an environment variable is how you
...
The configured homes first – `[[homes]]` in [`crowsnest.config`](crowsnest.config.html.md#module-crowsnest.config) is the naming
...
copies of *another machine’s* home, read-only by construction ([`crowsnest.config`](crowsnest.config.html.md#module-crowsnest.config):
