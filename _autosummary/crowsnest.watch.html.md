...
and `Notification` hooks (see [`crowsnest.hook`](crowsnest.hook.html.md#module-crowsnest.hook)), those two moments are *pushed*
...
([`crowsnest.rows.RowContext`](crowsnest.rows.html.md#crowsnest.rows.RowContext); by default the config file’s). Like the report’s,
...
`row_context` reaches [`attention_wakes()`](#crowsnest.watch.attention_wakes) ([`crowsnest.rows.RowContext`](crowsnest.rows.html.md#crowsnest.rows.RowContext)),
...
With `all_homes` every configured home is read (see [`crowsnest.config`](crowsnest.config.html.md#module-crowsnest.config)), each
with its own liveness rule, and every record carries its home’s name.

* **Return type:**
  [`dict`](https://docs.python.org/3/builtins/stdtypes.html#dict)[[`str`](https://docs.python.org/3/builtins/stdtypes.html#str), [`LiveSession`](crowsnest.registry.html.md#crowsnest.registry.LiveSession)]
