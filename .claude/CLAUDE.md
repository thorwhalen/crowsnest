...
| `identity=` in `attention.item_id`, `(row) -> tuple[str, ...]`, kind first. A field of `row_context=` | `("session", session_id)`, `uuid5(NAMESPACE, ":".join(...))`; only the last component may hold a colon. A row with `item_kind="question"` (#129) is `("question", session_id, prompt_uuid, k)`; a custom identity must handle it too, or questions lose their marks | `("ask", session_id, ask)` once the report and the verbs make one item per ask (today, one per row: `identity(row)` yields one id); a verdict's `asks` carry the text and time each would need, but an ask's text is raw, so the id would need it normalised; `("ref", url)` for an item several sessions share |
| `material=` in `attention.fingerprint`, `(row) -> tuple` | a question row's is `("question", state, answer_hash, answered_by)`: it changes when its answer does. Otherwise `(group, why, *normalised asks)`: a `needs_you` verdict's `asks` (`triage.Ask`, each whole), else its reason; a `working` row's reason (the tool in flight) left out; rows with no verdict or `unclassified`: `(status,)` plus an idle row's last words. One ask is the same three-part tuple a reason made, so such revisions survived #67. **Not the row's links**: they are resolved from tail text and the hook-rewritten `last_said`, and made chatter a change (adversarial review of #65). A field of `row_context=` | a tighter or looser tuple once resurfacing is measured (K2 in discussion #51) |
...
| `themes=` in `render_report` / `tools.report` / `tools.groups`: the person's `[themes]` table, `{theme: [owner/name, name, glob, org:owner]}` or a `themes.Themes` | `tools` reads `config.theme_table()`; `render_report` infers from the rows alone. `themes.infer` then places every row: the table, the repo owner, the parent (`parents_of(lineage)`), the refs' majority repo, `unthemed`. Nothing user-specific is a default: orgs and directory maps live in the person's table | a shared team map; openloops' project registry |
...
- **Every register is a `<details>`, built by `_register`** (#86) -- *Later* and *Review* included. A register with rows folds; one with none stays a `<section>`, because there is nothing to hide. `_register_from_rows` decides that (`folds=bool(rows)`) and a new register passes only `start_open=`, which the one that needs the person carries. Two things a `<summary>` may not hold: a block, so the rule is a `<span>` placed by `REGISTER_CSS`, and interactive content, so *Seen above* opens the body instead. **A folded register hides its rows from the person, not from the script**: anything that acts on rows in bulk (that button) must skip `details:not([open])`, or it decides for work nobody saw. `tests/test_report.py` and `tests/test_report_console.py` pin these. **The head's markup is openloops'** (#87 / openloops#12): `_register` is a wrapper that decides *Seen above* and hands the rest to `openloops.dashboard.register`, whose `folds=`/`start_open=`/`extra=` carry both trees, and `_rail` does the same with `.rail` -- so `report.py` holds no `<section class="register`, `<summary class="register-head"`, `<div class="register-head">` or `<div class="rail">` of its own, and `tests/test_report.py` fails if one comes back. **The head's *layout* is still stated in two places** and holds only while they agree: `openloops.dashboard.CSS` lays `.register-head` out as `auto 1fr`, and `REGISTER_CSS` places the folding head's three children into it (figure down column one, heading over rule in column two). The console's `laterBlock()` builds the head a third time, in DOM rather than markup, so the Python builder cannot reach it ([#94](https://github.com/thorwhalen/crowsnest/issues/94)); it is part of "change both together" below. And `OVERVIEW_CSS` (#109) restyles the *empty* head (`.register:has(>.empty)`: small figure, rule hidden) so a register with nothing in it is one line; that is a third statement of the head's layout, scoped to the `<section>` form, and moves to openloops with the other two when openloops#13 lands.
- **The board is an index, never a second copy** (the action-first pass, B1). `_board` draws one tile per shown session, each an `<a href="#session-…">` to its row, and its legend is the registers' figures in page order (the tally strip it replaced, #109). A tile's static state is `tile--seen`, the arm's `is-seen` (set by `followTile` in the console script), so an empty store still renders no attention class. Seen dims in place: `_in_order` never reads the store.
- Transcript *content* parsing is openloops' `parse_session`; never re-implement it here.
- **Your questions (#129) reads the person's own prompts and the replies to them**, through `openloops.exchanges`, never a parser here. `questions.py` caches each transcript's questions under `data_dir()/questions/files/` (local, keyed by path, invalidated by size and mtime). The register sanitises a message and an answer line by line *before* clipping (`_published_lines`, #83), and question rows join the attention view as `also=` items: their marks apply on any page, and they never decide whether the sessions' do. Its *All read* is `data-seen-above="questions"`, scoped to the register: the page's own *Seen above* would mark Needs you too.
- `links.py` never fetches, and neither does a render: a reference's state and title come from
  `refstate`'s store, which only a scheduled `refresh` fills; past `stale_after` it reads as unknown.
