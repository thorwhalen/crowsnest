...
At the foot, a closed **Review** block gathers what has sat too long, one line per session, grouped by kind: put off `max_snoozes` times or more and back again; seen, still needing the user, and untouched for `stale_after`; working in one status for `stuck_after` (timed by the status, so the line says only how long); handled here over `stuck_after` ago while the session never moved; and unclassified. Every threshold is the config file's `[attention]` table, and each heading states its own. The block counts toward nothing and wakes nobody. It appears only on a page that applies the store, so an empty store, `--plain` and `--no-triage` pages have none. When the user asks what to tidy, or for a quick review, point them at it.
...
The published page and the interactive console carry the open helper (`--open-helper` on `crowsnest report`; always on with `--interactive`): links open in new tabs, each account's sessions can be sent to the browser it is signed in to (the reader's choice, kept in their browser), and a session without Remote Control gets a button that copies its `crowsnest open` command. When the user says a session will not open from the page, point them at *Where sessions open* at the foot, or at turning Remote Control on in that session.
