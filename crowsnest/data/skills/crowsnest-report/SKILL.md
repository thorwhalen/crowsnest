...
At the foot, a closed **Review** block gathers what has sat too long, one line per session, grouped by kind: put off `max_snoozes` times or more and back again; seen, still needing the user, and untouched for `stale_after`; working with no change for `stuck_after`; handled here over `stuck_after` ago while the session never moved; and unclassified. Every threshold is the config file's `[attention]` table, and each heading states its own. The block counts toward nothing and wakes nobody. It appears only on a page that applies the store, so an empty store, `--plain` and `--no-triage` pages have none. When the user asks what to tidy, or for a quick review, point them at it.

A mark holds only while the page, the verbs and `crowsnest watch` triage each session from the same ledgers. When they are not in the default place, name the directory once in the config file as `[report] ledger_dir`, which all three read, rather than passing `--ledger-dir` to one of them. A flag given to the page and forgotten on a verb makes every item that verb marks read as changed.
...
**The Review block's lines carry the same buttons, scoped to what each kind needs** (#59): Drop, Later, Open on something put off too often; Answer (a link to the row), Later, Done on a stale ask; Ask, Later on a stuck or unclassified session; Undo (back on the page, seen) and Tell on something handled that never moved. Drop, Later, Done and Undo write the record as above. Ask and Tell queue the same intents a row's buttons do, so handle them the same way. A line tapped dims; it leaves the block when the page is next rendered.
